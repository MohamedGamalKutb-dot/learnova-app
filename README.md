...
1. **التخزين محلي** — جميع البيانات تُخزن في `localStorage` (لا يوجد Backend)
2. **الذكاء الاصطناعي اختياري** — التطبيق يعمل بدون API Key (يستخدم قاعدة المعرفة المحلية)
3. **كود الطفل فريد** — يتكون من `LN-` + 6 أحرف/أرقام عشوائية
4. **مستوى التشخيص** — لا يمكن للطفل أو ولي الأمر تغييره (الطبيب فقط)
5. **المزامنة** — البيانات تتزامن بين التبويبات عبر `storage` event و Polling

...
export function AuthProvider({ children }) {
    ...
    const loginChild = useCallback((identifier, password, isGoogle = false) => {
        const freshChildren = loadChildren();
    ...
    const loginParent = useCallback((email, password, isGoogle = false) => {
    ...
    const loginDoctor = useCallback((email, password, isGoogle = false) => {
        const existing = loadDoctors();
    ...
    const deleteAccount = useCallback(async (role, id) => {
        // Import inline or top to avoid circular deps, let's assume auth is available from config
        try {
        ...
        if (role === 'child') {
            const existing = loadChildren();
            saveChildren(existing.filter(c => c.childId !== id));
            setChildAccounts(loadChildren());
            logoutChild();
        } else if (role === 'parent') {
            const existing = loadParents();
            saveParents(existing.filter(p => p.id !== id));
            setParentAccounts(loadParents());
            logoutParent();
        } else if (role === 'doctor') {
            const existing = loadDoctors();
            saveDoctors(existing.filter(d => d.id !== id));
            setDoctorAccounts(loadDoctors());
            logoutDoctor();
        }
        return { success: true };
    }, [logoutChild, logoutParent, logoutDoctor]);

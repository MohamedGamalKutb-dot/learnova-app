...
function buildLocalFallback(): GlobalDataContextType {
...
export function GlobalDataProvider({ children }: { children: ReactNode }) {
    const [globalData, setGlobalData] = useState<GlobalDataContextType>({
    ...
    useEffect(() => {
        ...
        async function loadAllData() {
            try {
                // 1. PECS
                const pecsItemsDb = (await getDocs(collection(db, 'pecs_items'))).docs.map((d: any) => ({ ...d.data(), id: d.id }));
                ...
                // 2. Routine
                const routineItems = (await getDocs(collection(db, 'routine_data'))).docs.map((d: any) => ({ ...d.data(), id: d.id }));
                ...
                // 3. Clinics
                const clinicsDataDb = (await getDocs(collection(db, 'clinics_data'))).docs.map((d: any) => ({ ...d.data(), id: d.id }));
                ...
                // 4. Emotions
                const emotionsData = (await getDocs(collection(db, 'emotions_data'))).docs.map((d: any) => ({ ...d.data(), id: d.id }));

                // 5. KB
                const kbData = (await getDocs(collection(db, 'knowledge_base'))).docs.map((d: any) => d.data());
                ...
                // 6. Child Bot
                const childBotDataDb = (await getDocs(collection(db, 'child_bot_data'))).docs.map((d: any) => d.data());
                ...
                if (appData && appData.ar && appData.ar.landingData && appData.ar.landingData.heroCards) {
                    ...
                    appData.ar.landingData.heroCards.forEach((c: any) => {
                    ...
                    if (appData.en && appData.en.landingData && appData.en.landingData.heroCards) {
                        appData.en.landingData.heroCards.forEach((c: any) => {
                            if (c.icon && c.icon.includes('upload')) {

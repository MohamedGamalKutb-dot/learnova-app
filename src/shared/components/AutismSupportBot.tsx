...
import { getGeminiResponse, isGeminiAvailable } from '@/shared/services/geminiAI';

const normalize = (text: string) => text.toLowerCase().replace(/[ًٌٍَُِّْ]/g, '').replace(/[أإآ]/g, 'ا').replace(/[ة]/g, 'ه').replace(/[ى]/g, 'ي').trim();

interface AutismSupportBotProps {
    mode?: 'child' | 'parent' | 'doctor';
}

export default function AutismSupportBot({ mode = 'parent' }: AutismSupportBotProps) {
    const { isDark, isArabic } = useApp();
    const isChild = mode === 'child';
    const isDoctor = mode === 'doctor';
    const { knowledgeBase: kb, childBot } = useGlobalData();
    const { data: autismKnowledgeBase, defaultResponse, searchFallback } = kb || {};
    const { data: childBotData, childDefaultResponse, childFallbackResponse } = childBot || {};
    ...
    const [messages, setMessages] = useState<{ id: number; text: string; sender: 'bot' | 'user'; type: string; source: string; query: string }[]>([{ id: 1, text: initialText, sender: 'bot', type: 'answer', source: 'local', query: '' }]);
    ...
    const findLocalAnswer = useCallback((query: string) => {
        const normalizedQuery = normalize(query);
        const queryWords = normalizedQuery.split(/\s+/).filter(w => w.length > 0);
        let bestMatch: any = null, highestScore = 0;
        knowledgeBase.forEach((topic: any) => {
            let score = 0;
            if (topic.keywords) {
    ...
    const handleSend = async (textOverride: string | null = null) => {
        ...
        const userMsg = { id: Date.now(), text: textToSend, sender: 'user' as const, type: 'answer', source: 'local', query: '' };
        ...
        try {
            if (isGeminiAvailable()) {
                ...
                setMessages(prev => [...prev, { id: Date.now() + 1, text: aiResponse, sender: 'bot', type: 'answer', source: 'ai', query: '' }]);
                setIsTyping(false); return;
            }
        } catch (error: any) { console.warn('Gemini AI failed, falling back to local KB:', error.message); }
        setTimeout(() => {
            const localResponse = findLocalAnswer(textToSend);
            if (localResponse) setMessages(prev => [...prev, { id: Date.now() + 1, text: localResponse.text, sender: 'bot', type: 'answer', source: 'local', query: '' }]);
            else if (isChild) setMessages(prev => [...prev, { id: Date.now() + 1, text: fallbackText, sender: 'bot', type: 'answer', source: 'local', query: '' }]);
    ...
    return (
        <div className={`rounded-[20px] flex flex-col overflow-hidden my-5 shadow-[0_4px_20px_rgba(0,0,0,0.05)] border ${isDark ? 'bg-[#1F2940] border-[#444]' : 'bg-white border-gray-200'}`}
            ...
            <div className={`py-4 px-5 flex items-center gap-3 border-b ${isDark ? 'bg-[#16213E] border-[#333]' : 'bg-[#fafafa] border-gray-200'}`}>
                ...
                <button onClick={() => setMessages([{ id: 1, text: initialText, sender: 'bot', type: 'answer', source: 'local', query: '' }])} title={isArabic ? 'مسح المحادثة' : 'Clear Chat'} className="bg-transparent border-none cursor-pointer text-lg text-gray-400">🗑️</button>
            </div>

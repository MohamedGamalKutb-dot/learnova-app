...
export default function ClinicsMap() {
    ...
    const cities = useMemo(() => {
        ...
        return Array.from(new Set(clinicsData.map((c: any) => c.city)));
    }, [clinicsData]);
    ...
    const handleSearch = useCallback(async () => {
        ...
        const cityMatch = egyptCities.find((c: any) => c.name.toLowerCase().includes(q) || c.nameAr.includes(searchQuery.trim()));
        if (cityMatch) { 
        ...
        const clinicMatch = clinicsData.find((c: any) => 
    ...
    }, [searchQuery, egyptCities, clinicsData]);

    const filteredClinics = useMemo(() => {
        let result = clinicsData || [];
        if (selectedCity) result = result.filter((c: any) => c.city === selectedCity);
        if (filterService !== 'all') result = result.filter((c: any) => c.services.includes(filterService));
        return result.map((c: any) => ({ ...c, distance: getDistance(mapCenter[0], mapCenter[1], c.lat, c.lng) })).sort((a: any, b: any) => a.distance - b.distance);
    }, [clinicsData, selectedCity, filterService, mapCenter]);

    const handleCityClick = useCallback((city: string) => { const d = egyptCities.find((c: any) => c.name === city || c.nameAr === city); if (d) { setMapCenter([d.lat, d.lng]); setMapZoom(12); setSelectedCity(city); } }, [egyptCities]);
        ...
        <div className={`rounded-[40px] border-none shadow-none ${isDark ? 'bg-transparent text-white' : 'bg-transparent text-[#0C0D17]'}`}>
            ...
            <div className={`sticky top-[75px] z-20 pb-6 mb-12 pt-8 px-8 border-b ${isDark ? 'bg-[#0E101F]/95 backdrop-blur-3xl border-white/5' : 'bg-white/95 backdrop-blur-3xl border-slate-100 shadow-md'}`}>
                    ...
                    <div className="flex flex-col">
                        ...
                        {selectedCity && (
                            <p className="text-[11px] font-black uppercase tracking-widest text-indigo-400 mt-1">
                                {isArabic ? `عرض النتائج في ${egyptCities.find((c: any) => c.name === selectedCity)?.nameAr || selectedCity}` : `Showing results in ${selectedCity}`}
                            </p>
                        )}
                    </div>
                </div>
                ...
                <div className="flex gap-2 overflow-x-auto [scrollbar-width:none] mb-4 pb-1">
                    {['Cairo', 'Giza', 'Alexandria', 'Dakahlia', 'Gharbia', 'Sharqia', 'Assiut', 'Luxor', 'Suez'].map(city => (
                        <button key={city} onClick={() => handleCityClick(city)}
                            ...
                            {isArabic ? (egyptCities.find((c: any) => c.name === city)?.nameAr || city) : city}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex gap-2 p-1 rounded-2xl bg-white/5 border border-white/5 overflow-x-auto [scrollbar-width:none]">
                    ...
                    {Object.entries(serviceLabels).map(([key, label]: [string, any]) => (
            ...
            <div className="px-8 pt-10 pb-10 space-y-12">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredClinics.slice(0, 15).map((clinic: any, i: number) => (
                            <Card key={clinic.id} isPressable onPress={() => { setMapCenter([clinic.lat, clinic.lng]); setMapZoom(15); window.scrollTo({top: 500, behavior: 'smooth'}); }}
                                ...
                                <CardBody className="p-6">
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mt-4">
                                        {clinic.services.slice(0, 2).map((s: string) => (

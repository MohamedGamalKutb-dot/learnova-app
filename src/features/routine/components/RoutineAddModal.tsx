...
export default function RoutineAddModal({ isDark, isArabic, showAddModal, setShowAddModal, newItem, setNewItem, addItem, availableIcons, timeOfDayLabels, timeOfDayLabelsAr }: RoutineAddModalProps) {
    return (
                    ...
                    <>
                        ...
                        <ModalBody className="pb-8 pt-4 px-8 space-y-6">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {availableIcons.map(iconObj => (
                                        <Button key={iconObj.id} isIconOnly radius="lg" variant={newItem.iconId === iconObj.id ? "flat" : "bordered"} onPress={() => setNewItem((p: any) => ({ ...p, iconId: iconObj.id, emoji: iconObj.emoji }))}
                            ...
                            <div className="space-y-4">
                                <Input variant="underlined" label={isArabic ? 'اسم النشاط' : 'Activity Name'} value={newItem.title} onChange={e => setNewItem((p: any) => ({ ...p, title: e.target.value }))}
                                    classNames={{ label: "font-black text-[10px] opacity-40 uppercase tracking-widest" }} />
                                <Input variant="underlined" label={isArabic ? 'الوقت (اختياري)' : 'Time (Optional)'} placeholder="08:00 AM" value={newItem.startTime} onChange={e => setNewItem((p: any) => ({ ...p, startTime: e.target.value }))}
                                    classNames={{ label: "font-black text-[10px] opacity-40 uppercase tracking-widest" }} />
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    {['morning', 'afternoon', 'evening', 'night'].map(tod => (
                                        <Button key={tod} radius="full" size="sm" onPress={() => setNewItem((p: any) => ({ ...p, timeOfDay: tod }))}

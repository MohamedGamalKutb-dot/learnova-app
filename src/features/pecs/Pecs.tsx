...
export default function Pecs() {
    ...
    const getItemsByCategory = useCallback((category: string) => {
        return allItems ? allItems.filter((item: any) => item.category === category) : [];
    }, [allItems]);
    ...
    const updateSentence = useCallback((newSentence: any[]) => {
    ...
    const addToSentence = useCallback((item: any) => {
    ...
    const speakSentence = useCallback(() => {
        ...
        const t = sentence.map((s: any) => isArabic ? s.labelAr : s.label).join('. ');
        if ('speechSynthesis' in window) {

...
export default function Piano() {
    ...
    const [savedMelodies, setSavedMelodies] = useState<any[]>([]);
    const [stats, setStats] = useState({ totalNotes: 0, melodiesSaved: 0 });
    const [octaveShift, setOctaveShift] = useState(0);

    const [pianoConfig, setPianoConfig] = useState<{ notes: any[], keyColors: any, keyToCode: any }>({
        notes: [],
        keyColors: {},
        keyToCode: {}
    });
    ...
    useEffect(() => {
        ...
        getPianoConfig().then((config: any) => {
    ...
    useEffect(() => {
        ...
        const unsubStats = subscribeToGameStats(currentChild.childId, (freshStats: any) => {
        ...
        const unsubMelodies = subscribeToMelodies(currentChild.childId, (freshMelodies: any) => {
    ...
    const playNote = useCallback((noteObj: any) => {
        try {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            const noteObj = NOTES.find((n: any) => n.key === pressedKey || KEY_TO_CODE[n.key] === pressedCode);
            if (noteObj) {
                e.preventDefault();
                playNote(noteObj);
            }
    ...
    const handlePlayRecording = async () => {
        ...
        for (let i = 0; i < recording.length; i++) {
            ...
            const noteObj = NOTES.find((n: any) => n.note === note.note);
            if (!noteObj) continue;
    ...
    const handleDeleteMelody = (melodyId: string) => {
        if (!currentChild?.childId) return;
        deleteMelody(currentChild.childId, melodyId);
    };

    const handlePlaySavedMelody = async (melodyNotes: any[]) => {
        if (isPlaying) return;
        setIsPlaying(true);
        onMelodiesClose();

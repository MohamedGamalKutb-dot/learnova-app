...
export default function ArtStudio() {
    ...
    const [drawingConfig, setDrawingConfig] = useState<any>({
        COLORS: [],
        BRUSH_SIZES: [],
        TOOLS: {},
        SHAPES: {}
    ...
    const stampShapesRef = useRef<any[]>([]);
    ...
    useEffect(() => {
        ...
        const unsub = subscribeToDrawings(currentChild.childId, (freshDrawings: any[]) => {
    ...
    const handleDrawShape = (shapeKey: string) => {
        const canvas = canvasRef.current;
        if (!canvas) return;
        const rect = canvas.getBoundingClientRect();
    ...
    const handleLoadDrawing = (drawingData: string) => {
        const img = new Image();
        img.onload = () => {
            const canvas = canvasRef.current;
            const ctx = ctxRef.current;

...
interface AssessmentTabProps {
    ...
    accent: string;
    selectedPatient: any;
    viewingAssessment: any;
    setViewingAssessment: (ass: any) => void;
    assessmentDone: boolean;
    setAssessmentDone: (done: boolean) => void;
    assessmentQuestions: any[];
    assessmentAnswers: Record<string, string>;
    setAssessmentAnswers: (updater: (prev: Record<string, string>) => Record<string, string>) => void;
    submitAssessment: () => void;
    setHoveredCard: (id: string | null) => void;
    cardCls: (id: string | null) => string;
    ...
    if (assessmentDone) {
        const last = selectedPatient.assessments?.length > 0 ? selectedPatient.assessments[selectedPatient.assessments.length - 1] : { score: 0 }; return (
            <Card className={cardCls(null)}><CardBody className="text-center p-5"><div className="text-6xl mb-4 text-emerald-500 flex justify-center"><FaCheckCircle /></div><h2 className={`mb-2 font-bold ${isDark ? 'text-text-dark' : 'text-text'}`}>{isArabic ? 'تم حفظ التقييم!' : 'Assessment Saved!'}</h2><div className="text-[40px] font-extrabold text-accent">{last.score}%</div>
                <Button radius="lg" size="lg" className="mt-5 bg-gradient-to-br from-accent to-[#4834D4] text-white font-bold" onPress={() => { setAssessmentDone(false); setAssessmentAnswers(() => ({})); }}>{isArabic ? 'تقييم جديد' : 'New Assessment'}</Button>
            </CardBody></Card>
        );
    }
    return (
        <div>
            ...
            <Button fullWidth radius="lg" size="lg" onPress={submitAssessment} isDisabled={Object.keys(assessmentAnswers).length < assessmentQuestions.length}
            ...
            {selectedPatient.assessments?.length > 0 && (
                <div><h4 className={`mb-3 pt-4 text-[15px] font-bold border-t ${isDark ? 'text-text-dark border-border-dark' : 'text-text border-border'}`}>{isArabic ? 'التقييمات السابقة' : 'Previous Assessments'}</h4>
                    {[...(selectedPatient.assessments || [])].reverse().map((ass: any, idx: number) => (

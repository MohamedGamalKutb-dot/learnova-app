...
interface BehaviorTabProps {
    ...
    behaviorTypes: any[];
    behaviorType: string;
    setBehaviorType: (type: string) => void;
    behaviorIntensity: number;
    setBehaviorIntensity: (intensity: number) => void;
    behaviorNote: string;
    setBehaviorNote: (note: string) => void;
    addBehaviorLog: () => void;
        ...
        <div>
            ...
            <Card className={cardCls('newBehavior')} onMouseEnter={() => setHoveredCard('newBehavior')} onMouseLeave={() => setHoveredCard(null)}>
                <CardBody className="p-[22px]">
                    ...
                    <Button fullWidth radius="lg" size="lg" onPress={addBehaviorLog} className="bg-gradient-to-br from-[#FF6584] to-pink-500 text-white font-bold text-sm shadow-[0_4px_12px_rgba(255,101,132,0.25)] transition-all duration-200 hover:-translate-y-px mt-2">
            ...
            {(!selectedPatient.behaviorLogs || selectedPatient.behaviorLogs.length === 0) ? <p className={`text-[13px] text-center p-4 ${isDark ? 'text-subtext-dark' : 'text-subtext'}`}>{isArabic ? 'لا توجد سجلات بعد' : 'No logs yet'}</p> : (
                [...(selectedPatient.behaviorLogs || [])].reverse().map((log: any, i: number) => {

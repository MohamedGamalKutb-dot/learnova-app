...
interface LandingLoginSectionProps {
    ...
    loginCards: { path: string; topC: string; iconBg: string; icon: string; h: string; p: string; btnBg: string; btn: string }[];
    navigate: (path: string) => void;
}

export default function LandingLoginSection({ isDark, T, loginCards, navigate }: LandingLoginSectionProps) {
    ...
    return (
        <section id="login" className={`py-16 md:py-[88px] px-5 md:px-14 ${darkBg2}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[960px] mx-auto">
                {loginCards.map((c, i) => (
                    <Card key={i} onClick={() => navigate(c.path)}
                        className={`${darkSurf} border ${darkBdr} rounded-2xl pt-10 px-7 pb-8 text-center transition-all duration-300 relative overflow-hidden hover:shadow-[0_24px_56px_rgba(37,99,235,.10)]`}>
                        <div className={`absolute top-0 inset-x-0 h-1 rounded-t-2xl bg-gradient-to-r ${c.topC}`} />
                        <CardBody className="p-0 flex flex-col items-center overflow-hidden">
                            ...
                            <Button size="sm" radius="lg" className={`${c.btnBg} text-white font-bold font-jakarta`} onPress={() => navigate(c.path)}>{c.btn}</Button>
                        </CardBody>
                    </Card>
                ))}
            </div>

...
import { FaJournalWhills, FaTrash, FaInbox, FaBookOpen } from 'react-icons/fa';

interface SanctuaryJournalTabProps {
    isArabic: boolean;
    isDark: boolean;
    auraCard: string;
            ...
            <div className="space-y-6">
                    ...
                    <Card className={`rounded-[45px] border animate-in slide-in-from-top-4 duration-500 ${auraCard}`}>
                        <CardBody className="p-5 sm:p-8">
                            <Textarea variant="flat" placeholder={isArabic ? 'كيف كان يوم بطلك؟ شاركنا ملاحظاتك...' : 'Record today\'s observations or milestones...'}
                                value={noteText} onChange={e => setNoteText(e.target.value)} size="lg" radius="lg"
                ...
                <div className="space-y-5">
                    {data?.dailyNotes && data.dailyNotes.length > 0 ? (
                        [...data.dailyNotes].reverse().map((note: any, idx: number) => (

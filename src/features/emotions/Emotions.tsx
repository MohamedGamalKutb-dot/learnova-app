...
export default function Emotions() {
    ...
    const { emotions: globalEmotions, isLoading } = useGlobalData();
    const allEmotions = globalEmotions?.allEmotions || [];
    ...
    const emotions = allEmotions.filter((e: any) => e.difficultyLevel <= currentLevel);
    const currentEmotion = emotions[currentIndex] || emotions[0] || {};
    const historyEntries = Object.entries(currentChild?.emotionHistory || {}).sort((a: any, b: any) => new Date(b[0]).getTime() - new Date(a[0]).getTime()).slice(0, 5);
    ...
    const startQuiz = () => {
        ...
        const questions = finalSelection.map((answer: any) => {
            const options = [answer, ...emotions.filter((e: any) => e.id !== answer.id).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
    ...
    const nextQuiz = useCallback(() => {
        if (currentQuestionIdx >= quizQuestions.length - 1) {
            ...
            setCurrentQuestionIdx((p: any) => p + 1);
    ...
    const answerQuiz = (optionId: string) => {
        ...
        if (currentChild && updateChildEmotionStats) {
            updateChildEmotionStats(currentChild.childId, todayKey, correct ? 1 : 0, 1);
        }

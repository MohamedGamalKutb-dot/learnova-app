...
interface WordGameFiltersProps {
    isArabic: boolean;
    wordGameConfig: any;
    difficulty: string;
...
export default function WordGameFilters({ isArabic, wordGameConfig, difficulty, setDifficulty, difficultyLabels, words, category, setCategory, categoryLabels }: WordGameFiltersProps) {
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        >
            {/* Difficulty chips */}
            {Object.keys(wordGameConfig.DIFFICULTY || {}).map((d: any) => (
                <Chip
                    key={d}

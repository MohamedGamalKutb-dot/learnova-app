...
function getRandomWord(words, difficulty, difficultyConfig) {
    if (!difficultyConfig) return Object.values(words).flat()[0] || '';
    const diff = difficultyConfig[difficulty] || { min: 3, max: 50 };
    const { min, max } = diff;
    const filtered = Object.values(words).flat().filter(w => w.length >= min && w.length <= max);
    if (filtered.length === 0) return Object.values(words).flat()[0] || '';
    return filtered[Math.floor(Math.random() * filtered.length)];
}

function splitLetters(word, isArabic) {
    if (isArabic) {
        return [...word];
    }
    return word.split('');
...
export default function WordGame() {
    ...
    const [wordsConfig, setWordsConfig] = useState({ words: {}, labels: {} });
    const [wordGameConfig, setWordGameConfig] = useState({
        DIFFICULTY: { easy: { min: 3, max: 4 } },
        encouragements: { ar: ['أحسنت!'], en: ['Great job!'] }
    });

    useEffect(() => {
        getWordGameConfig().then(config => {
            if (config) setWordGameConfig(config);
        });
    }, []);

    useEffect(() => {
        getWordsConfig(isArabic).then(config => {
            if (config) {
    ...
    useEffect(() => {
        if (currentChild?.childId) {
            ...
            setStats({
                correct: gameStats.wordCorrect,
                wrong: gameStats.wordWrong,
                score: gameStats.wordScore,
                streak: gameStats.wordStreak,
            });
        }
    }, [currentChild]);
    ...
    const loadNewWord = useCallback(() => {
        ...
        setCurrentWord(word);
        setShuffledLetters(shuffleArray(letters.map((l, i) => ({ letter: l, originalIndex: i, id: `${l}-${i}` }))));
        setSelectedIndices([]);
        setAnswer([]);
        setIsCorrect(null);
        setShowResult(false);
    ...
    const handleRemoveLetter = (answerIndex) => {
